...
3. Click **Run ▶**  
4. (Optional) The sketch saves a static output to `assets/output_image.png` via `saveFrame()`.

> 💡 If you don’t see Python Mode, install it via **Processing → Modes → Add Mode → Python Mode**.
...
- (Optional) **Mouse/keyboard** interaction to trigger new frames  
...
- **Functions** → modularise drawing behaviour  
...
- **Platform output:** Generates `.png` images and optional `.mp4` demo  
...
- Itheima.com. (2018). *UI设计之波普艺术的应用方式.*  
- Wikipedia Contributors. (2019). *Processing (programming language).* Wikimedia Foundation.  
...
Disadvantages: Occasionally misunderstood creative context, requiring manual correction.  

> Screenshots of AI prompts and outputs are included in the **Appendix** of the Written Reflection.
